...
async def generate_inkoopfactuur_pdf(factuur: dict, user: dict, crediteur: dict) -> BytesIO:
    """Generate a professional PDF for purchase invoices"""
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=A4,
    ...
    title_style = ParagraphStyle(
        ...
        parent=styles['Heading1'],
        fontSize=24,
        textColor=colors.HexColor('#dc2626'),  # red-600 for purchase
        spaceAfter=20,
        alignment=TA_LEFT
    )
    ...
    header_data = [
        [
            Paragraph(f"<b>{company_name}</b>", ParagraphStyle('Company', fontSize=16, textColor=colors.HexColor('#111827'))),
            Paragraph(f"<b>INKOOPFACTUUR</b>", ParagraphStyle('InvoiceTitle', fontSize=24, textColor=colors.HexColor('#dc2626'), alignment=TA_RIGHT))
        ]
    ]
    ...
    # Supplier info
    crediteur_naam = crediteur.get('naam') or crediteur.get('bedrijfsnaam') or factuur.get('crediteur_naam', 'Onbekend')
    crediteur_adres = crediteur.get('adres') or ''
    crediteur_stad = crediteur.get('stad') or ''
    crediteur_email = crediteur.get('email') or ''
    
    supplier_address = crediteur_naam
    if crediteur_adres:
        supplier_address += f"<br/>{crediteur_adres}"
    if crediteur_stad:
        supplier_address += f"<br/>{crediteur_stad}"
    if crediteur_email:
        supplier_address += f"<br/>{crediteur_email}"
    
    # SRD equivalent info
    srd_info = ""
    if factuur.get('srd_equivalent') and factuur.get('valuta') != 'SRD':
        srd_info = f"<br/><b>SRD Equivalent:</b> SRD {factuur.get('srd_equivalent'):,.2f}"
    
    # Kostenplaats info
    kostenplaats_info = ""
    if factuur.get('kostenplaats_naam'):
        kostenplaats_info = f"<br/><b>Kostenplaats:</b> {factuur.get('kostenplaats_naam')}"
    
    info_data = [
        [
            Paragraph("<b>Onze Gegevens:</b>", small_header),
            Paragraph("<b>Leverancier:</b>", small_header),
            Paragraph("<b>Factuur Details:</b>", small_header)
        ],
        [
            Paragraph(company_info or company_name, value_style),
            Paragraph(supplier_address, value_style),
            Paragraph(f"""
                <b>Intern Nr:</b> {factuur.get('factuurnummer', 'N/A')}<br/>
                <b>Lev. Factuur:</b> {factuur.get('factuurnummer_leverancier', '-')}<br/>
                <b>Factuurdatum:</b> {factuurdatum}<br/>
                <b>Vervaldatum:</b> {vervaldatum}<br/>
                <b>Valuta:</b> {factuur.get('valuta', 'SRD')}{srd_info}{kostenplaats_info}
            """, value_style)
        ]
    ]
    ...
    # Table header - red theme for purchases
    line_data = [
        [
            Paragraph("<b>Omschrijving</b>", ParagraphStyle('TH', fontSize=10, textColor=colors.white)),
            Paragraph("<b>Aantal</b>", ParagraphStyle('TH', fontSize=10, textColor=colors.white, alignment=TA_CENTER)),
    ...
    line_table = Table(line_data, colWidths=[7*cm, 2*cm, 3*cm, 2*cm, 3*cm])
    line_table.setStyle(TableStyle([
        # Header styling - red for purchase invoices
    ...
    totals_table.setStyle(TableStyle([
        ...
        ('FONTNAME', (2, -1), (3, -1), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, -1), 10),
        ('TEXTCOLOR', (2, -1), (3, -1), colors.HexColor('#dc2626')),
        ('LINEABOVE', (2, -1), (3, -1), 1, colors.HexColor('#dc2626')),
        ('TOPPADDING', (0, 0), (-1, -1), 4),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 4),
    ]))
    elements.append(totals_table)
    ...
    # ============ PAYMENT STATUS ============
    status = factuur.get('status', 'ontvangen')
    status_labels = {
        'ontvangen': 'Ontvangen',
        'gedeeltelijk_betaald': 'Gedeeltelijk Betaald',
        'betaald': 'Betaald',
    ...
    status_style = ParagraphStyle(
        'Status',
        fontSize=12,
        textColor=colors.HexColor('#059669') if status == 'betaald' else colors.HexColor('#f59e0b') if status in ['ontvangen', 'gedeeltelijk_betaald'] else colors.HexColor('#ef4444')
    )
    
    elements.append(Paragraph(f"<b>Betaalstatus:</b> {status_text}", status_style))
    elements.append(Spacer(1, 0.5*cm))
    ...
    elements.append(Spacer(1, 1*cm))
    elements.append(Paragraph("─" * 60, footer_style))
    elements.append(Paragraph(f"Inkoopfactuur geregistreerd door {company_name}", footer_style))
    elements.append(Paragraph(f"Gegenereerd op {datetime.now().strftime('%d-%m-%Y %H:%M')}", footer_style))
...
@router.get("/inkoopfactuur/{factuur_id}")
async def download_inkoopfactuur_pdf(
    factuur_id: str,
    current_user: dict = Depends(get_current_active_user)
):
    ...
    if not crediteur:
        crediteur = {"naam": factuur.get('crediteur_naam', 'Onbekend')}
    ...
    # Generate PDF
    pdf_buffer = await generate_inkoopfactuur_pdf(factuur, user, crediteur)
    
    # Return as downloadable file
    filename = f"Inkoopfactuur_{factuur.get('factuurnummer', factuur_id)}.pdf"
...
@router.get("/inkoopfactuur/{factuur_id}/preview")
async def preview_inkoopfactuur_pdf(
    factuur_id: str,
    current_user: dict = Depends(get_current_active_user)
):
    """Preview a purchase invoice as PDF (inline display)"""
    user_id = current_user["id"]
