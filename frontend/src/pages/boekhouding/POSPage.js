...
<body>
  ...
  return (
    <div className="fixed inset-0 bg-gray-100 flex" data-testid="pos-page">
      ...
      {/* Right Side - Cart + Integrated Payment */}
      <div className="w-[420px] lg:w-[480px] bg-white border-l border-gray-200 flex flex-col">
        {/* Cart Header */}
        <div className="p-3 border-b border-gray-100 bg-gray-50">
          ...
          {/* Selected Customer */}
          {selectedCustomer && (
            <div className="flex items-center justify-between bg-blue-50 rounded-lg px-2 py-1.5 mt-2">
              <div className="flex items-center gap-2">
                <User className="w-3 h-3 text-blue-500" />
                <span className="text-xs font-medium text-blue-700">{selectedCustomer.naam}</span>
              </div>
              <button 
        ...
        {/* Cart Items - Compact scrollable area */}
        <div className="h-40 overflow-y-auto p-2 space-y-1 border-b border-gray-100">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              <ShoppingCart className="w-8 h-8 mb-2" />
              <p className="text-xs">Winkelwagen is leeg</p>
            </div>
          ) : (
            cart.map(item => (
              <div 
              ...
              >
                ...
                <div className="flex items-center gap-0.5">
                  <button
                    ...
                    className="w-5 h-5 rounded-full border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-100"
                  >
                    <Minus className="w-2.5 h-2.5" />
                  </button>
                  <div className="w-6 h-6 rounded-full border border-gray-200 flex items-center justify-center text-xs font-medium text-gray-700">
                    {item.quantity}
                  </div>
                  <button
                    onClick={() => updateQuantity(item.id, 1)}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-gray-900 truncate">{item.naam}</p>
                  <p className="text-[10px] text-gray-400">{formatCurrency(item.verkoopprijs)} p/s</p>
                </div>
                ...
                <button
                  ...
                  className="w-5 h-5 rounded-full flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
        ...
        {/* Totals Section */}
        <div className="p-3 border-b border-gray-100 bg-gray-50 space-y-1">
          ...
          {appliedDiscount && (
            <div className="flex justify-between text-xs text-emerald-600">
              <span className="flex items-center gap-1">
                Korting
                <button onClick={removeDiscount} className="text-red-400 hover:text-red-600">
          ...
          {!appliedDiscount && cart.length > 0 && (
            <button
              onClick={() => setShowDiscountDialog(true)}
              className="w-full py-1 text-[10px] text-gray-400 hover:text-emerald-600 border border-dashed border-gray-200 rounded hover:border-emerald-300 flex items-center justify-center gap-1"
            >
        ...
        <div className="flex-1 p-3 flex flex-col">
          {/* Payment Method Selection or Numpad */}
          {!paymentMethod ? (
            /* Payment Method Selection */
            <div className="flex-1 flex flex-col">
              <p className="text-xs text-gray-500 uppercase tracking-wide mb-2 text-center">Betaalmethode</p>
              <div className="grid grid-cols-2 gap-2 flex-1">
                <button
                  onClick={() => cart.length > 0 && setPaymentMethod('contant')}
                ...
                >
                  <Banknote className={`w-10 h-10 ${cart.length > 0 ? 'text-emerald-600' : 'text-gray-300'}`} />
                  <span className={`text-sm font-semibold ${cart.length > 0 ? 'text-gray-700' : 'text-gray-400'}`}>Contant</span>
                </button>
                <button
                  onClick={() => cart.length > 0 && startPayment('pin')}
                  disabled={cart.length === 0}
                  className={`flex flex-col items-center justify-center gap-2 rounded-xl border-2 transition-all ${
                    cart.length > 0 
                ...
                >
                  <CreditCard className={`w-10 h-10 ${cart.length > 0 ? 'text-blue-600' : 'text-gray-300'}`} />
                  <span className={`text-sm font-semibold ${cart.length > 0 ? 'text-gray-700' : 'text-gray-400'}`}>Pin/Kaart</span>
                </button>
              </div>
            </div>
          ) : paymentMethod === 'contant' && paymentStep !== 'processing' && paymentStep !== 'success' ? (
            /* Cash Payment with Numpad - Integrated */
            <div className="flex-1 flex flex-col">
              {/* Back button */}
              <button 
              ...
              <div className="grid grid-cols-3 gap-2 mb-2">
                <div className="bg-gray-900 text-white p-2 rounded-lg text-center">
                  <p className="text-[9px] text-gray-400 uppercase">Te Betalen</p>
                  <p className="text-sm font-bold font-mono">{formatCurrency(total)}</p>
                </div>
                <div className="bg-emerald-600 text-white p-2 rounded-lg text-center">
                  <p className="text-[9px] text-emerald-200 uppercase">Ontvangen</p>
                  <p className="text-sm font-bold font-mono">{formatCurrency(cashReceivedNum || 0)}</p>
                </div>
                <div className={`p-2 rounded-lg text-center ${
                  ...
                  <p className={`text-[9px] uppercase ${cashReceivedNum >= total ? 'text-emerald-600' : 'text-amber-600'}`}>
                    {cashReceivedNum >= total ? 'Wisselgeld' : 'Nog nodig'}
                  </p>
                  <p className={`text-sm font-bold font-mono ${cashReceivedNum >= total ? 'text-emerald-700' : 'text-amber-700'}`}>
              ...
              <div className="grid grid-cols-3 gap-1.5 mb-2">
                {[7, 8, 9, 4, 5, 6, 1, 2, 3].map(num => (
                  <button
                    ...
                    className="h-11 text-lg font-bold bg-gray-100 hover:bg-gray-200 active:bg-gray-300 rounded-lg transition-colors"
                    data-testid={`pos-numpad-${num}`}
                  >
                    {num}
                  </button>
                ))}
                <button
                  ...
                  className="h-11 text-sm font-bold bg-gray-100 hover:bg-gray-200 active:bg-gray-300 rounded-lg transition-colors"
                  data-testid="pos-numpad-00"
                >
                  00
                </button>
                <button
              ...
              <div className="grid grid-cols-5 gap-1 mb-2">
                {[10, 20, 50, 100, 200].map(amount => (
                  <button
                    ...
                    className="py-1.5 bg-blue-50 hover:bg-blue-100 active:bg-blue-200 rounded text-xs font-bold text-blue-700 transition-colors"
                    data-testid={`pos-quick-${amount}`}
                  >
                    {amount}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-3 gap-1 mb-2">
                <button
                  ...
                  className="py-1.5 bg-emerald-100 hover:bg-emerald-200 rounded text-xs font-bold text-emerald-700"
                  data-testid="pos-exact-btn"
                >
                  Exact
                </button>
                <button
                  onClick={() => setCashReceived(prev => prev.slice(0, -1) || '0')}
                  className="py-1.5 bg-amber-100 hover:bg-amber-200 rounded text-xs font-bold text-amber-700 flex items-center justify-center gap-1"
                  data-testid="pos-backspace-btn"
                >
                  <ArrowLeft className="w-3 h-3" /> Wis
                </button>
                <button
                  onClick={() => setCashReceived('0')}
                  className="py-1.5 bg-red-100 hover:bg-red-200 rounded text-xs font-bold text-red-700"
                  data-testid="pos-reset-btn"
                >
                  Reset
                </button>
              </div>
              ...
              >
                ...
                {cashReceivedNum >= total ? 'Betaling Bevestigen' : `Nog ${formatCurrency(total - cashReceivedNum)}`}
              </Button>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-gray-600">Betaling verwerken...</p>
            </div>
          ) : paymentStep === 'success' ? (
            /* Success State */
            <div className="flex-1 flex flex-col">
              <div className="text-center mb-4">
                ...
                <h2 className="text-lg font-bold text-gray-900">Betaling Succesvol!</h2>
                <p className="text-xs text-gray-500">{lastSale?.bonnummer}</p>
              </div>
              
              {/* Summary */}
              <div className="bg-gray-50 rounded-lg p-3 space-y-1.5 text-xs mb-4">
                ...
                {paymentMethod === 'contant' && lastSale?.wisselgeld > 0 && (
                  <>
                    ...
                    <div className="flex justify-between text-emerald-600 font-semibold pt-1.5 border-t border-gray-200">
                      <span>Wisselgeld</span>
                      <span className="text-base">{formatCurrency(lastSale?.wisselgeld)}</span>
                    </div>
                  </>
                )}
              </div>
