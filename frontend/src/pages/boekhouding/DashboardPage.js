...
export default function BoekhoudingDashboard() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 px-2 sm:px-0" data-testid="boekhouding-dashboard">
      ...
      {/* Valuta Exposure Warning Widget */}
      {exposure && exposure.per_valuta?.length > 0 && (
        <div className={`rounded-xl sm:rounded-2xl border p-4 sm:p-6 ${
          Math.abs(exposure.totaal_netto_exposure_srd || 0) > EXPOSURE_WARNING_THRESHOLD 
          ...
          <div className="flex flex-col sm:flex-row items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className={`text-xs sm:text-sm mb-3 ${
                ...
                {Math.abs(exposure.totaal_netto_exposure_srd || 0) > EXPOSURE_WARNING_THRESHOLD 
                  ? 'U heeft een significante valutapositie. Overweeg hedging maatregelen.'
                  : 'Uw openstaande valutaposities zijn binnen normale limieten.'}
              </p>
              
              {/* Exposure per valuta */}
              <div className="flex flex-wrap gap-2 mb-3">
                {exposure.per_valuta.map((item) => (
                  <div key={item.valuta} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${
                    Math.abs(exposure.totaal_netto_exposure_srd || 0) > EXPOSURE_WARNING_THRESHOLD 
                      ? 'bg-amber-100 dark:bg-amber-900/30'
                      : 'bg-blue-100 dark:bg-blue-900/30'
                  }`}>
                    {item.valuta === 'EUR' ? <Euro className="w-4 h-4" /> : <DollarSign className="w-4 h-4" />}
                    <span className="text-sm font-medium">{item.valuta}</span>
                    <span className={`text-sm font-bold ${item.netto >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                      {item.netto >= 0 ? '+' : ''}{item.netto?.toLocaleString('nl-NL')}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm">
                  <span className="text-muted-foreground">Totaal SRD equivalent: </span>
                  <span className={`font-bold ${
                    (exposure.totaal_netto_exposure_srd || 0) >= 0 ? 'text-emerald-600' : 'text-red-600'
                  }`}>
                    SRD {(exposure.totaal_netto_exposure_srd || 0).toLocaleString('nl-NL', { minimumFractionDigits: 2 })}
                  </span>
                </div>
                <Button 
                  variant="outline" 
                  size="sm"
                  onClick={() => navigate('/app/boekhouding/valuta-rapportages')}
                  className={`text-xs ${
                    Math.abs(exposure.totaal_netto_exposure_srd || 0) > EXPOSURE_WARNING_THRESHOLD 
                      ? 'border-amber-300 hover:bg-amber-100 dark:border-amber-700'
                      : 'border-blue-300 hover:bg-blue-100 dark:border-blue-700'
                  }`}
                >
