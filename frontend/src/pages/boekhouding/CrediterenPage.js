...
import { 
  Loader2, Plus, Pencil, Trash2, Building2, Search, 
  Mail, Phone, TrendingDown, FileText, DollarSign
} from 'lucide-react';
...
export default function CrediterenPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Kon crediteur niet verwijderen');
    }
  };
  ...
  const openEdit = (crediteur) => {
    setForm({
      naam: crediteur.naam || '',
      email: crediteur.email || '',
      telefoon: crediteur.telefoon || '',
      adres: crediteur.adres || '',
      btw_nummer: crediteur.btw_nummer || '',
      standaard_valuta: crediteur.standaard_valuta || 'SRD',
      betalingstermijn: crediteur.betalingstermijn || 30,
      notities: crediteur.notities || ''
    });
    setEditingId(crediteur.id);
    setDialogOpen(true);
  };
  ...
  const filteredCrediteuren = crediteuren.filter(c =>
    c.naam?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    c.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    c.crediteur_nummer?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const totalOpenstaand = crediteuren.reduce((sum, c) => sum + (c.openstaand_bedrag || 0), 0);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Crediteuren laden...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 px-2 sm:px-0" data-testid="crediteuren-page">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-purple-900 p-4 sm:p-6 lg:p-8">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-1">Crediteuren</h1>
            <p className="text-slate-400 text-sm sm:text-base">Beheer uw leveranciers en openstaande schulden</p>
          </div>
          <Button 
        ...
        <div className="rounded-xl sm:rounded-2xl bg-card border border-border/50 p-3 sm:p-4 lg:p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg sm:text-xl lg:text-2xl font-bold">{crediteuren.length}</p>
            </div>
          </div>
        </div>
        <div className="rounded-xl sm:rounded-2xl bg-card border border-border/50 p-3 sm:p-4 lg:p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs sm:text-sm text-muted-foreground">Te Betalen</p>
              <p className="text-lg sm:text-xl lg:text-2xl font-bold text-red-600 truncate">{formatCurrency(totalOpenstaand)}</p>
            </div>
          </div>
        </div>
        <div className="rounded-xl sm:rounded-2xl bg-card border border-border/50 p-3 sm:p-4 lg:p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg sm:text-xl lg:text-2xl font-bold">{crediteuren.reduce((s, c) => s + (c.facturen_count || 0), 0)}</p>
            </div>
          </div>
        </div>
        <div className="rounded-xl sm:rounded-2xl bg-card border border-border/50 p-3 sm:p-4 lg:p-5">
              ...
              <div key={crediteur.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 hover:bg-muted/50 transition-colors gap-4">
                <div className="flex items-start sm:items-center gap-3 sm:gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-sm sm:text-base">{crediteur.naam}</p>
                      {crediteur.crediteur_nummer && (
                        <Badge variant="outline" className="text-xs">{crediteur.crediteur_nummer}</Badge>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs sm:text-sm text-muted-foreground">
                      {crediteur.email && (
                        <span className="flex items-center gap-1">
                          <Mail className="w-3 h-3" /> {crediteur.email}
                        </span>
                      )}
                      {crediteur.telefoon && (
                        <span className="flex items-center gap-1">
                          <Phone className="w-3 h-3" /> {crediteur.telefoon}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 ml-13 sm:ml-0">
                  <div className="text-left sm:text-right">
                    ...
                    <p className={`font-bold text-base sm:text-lg ${(crediteur.openstaand_bedrag || 0) > 0 ? 'text-red-600' : ''}`}>
                      {formatCurrency(crediteur.openstaand_bedrag || 0, crediteur.standaard_valuta)}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => openEdit(crediteur)} className="rounded-lg">
                      <Pencil className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => handleDelete(crediteur.id)} className="rounded-lg text-red-500 hover:text-red-600 hover:bg-red-50">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl">{editingId ? 'Crediteur Bewerken' : 'Nieuwe Crediteur'}</DialogTitle>
          </DialogHeader>
