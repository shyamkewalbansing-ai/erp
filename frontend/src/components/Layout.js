...
// Nu met dropdown submodules: Inkoop, Verkoop, Voorraad, Projecten
// Workflow vereenvoudigd: Offerte → Factuur (zonder Orders tussenstap)

// Hoofditems (geen dropdown)
const boekhoudingMainItems = [
  ...
  { to: '/app/boekhouding/bankreconciliatie', icon: RefreshCw, label: 'Bank Reconciliatie', addon: 'boekhouding' },
...
// Dropdown submodules
const boekhoudingDropdowns = {
  inkoop: {
    label: 'Inkoop',
    icon: ShoppingCart,
    items: [
      { to: '/app/boekhouding/inkoop/offertes', icon: FileText, label: 'Inkoopoffertes' },
      { to: '/app/boekhouding/inkoop/ontvangsten', icon: Package, label: 'Goederenontvangst' },
      { to: '/app/boekhouding/inkoop/facturen', icon: FileText, label: 'Inkoopfacturen' },
    ]
  },
  verkoop: {
    label: 'Verkoop',
  ...
  voorraad: {
    ...
    icon: Package,
    items: [
      { to: '/app/boekhouding/voorraad/artikelen', icon: Package, label: 'Artikelen' },
      { to: '/app/boekhouding/voorraad/magazijnen', icon: Building2, label: 'Magazijnen' },
      { to: '/app/boekhouding/voorraad/mutaties', icon: Zap, label: 'Voorraadmutaties' },
      { to: '/app/boekhouding/voorraad/inventarisatie', icon: Target, label: 'Inventarisatie' },
    ]
  },
  projecten: {
    label: 'Projecten',
    icon: Briefcase,
    items: [
      { to: '/app/boekhouding/projecten/overzicht', icon: Briefcase, label: 'Alle Projecten' },
      { to: '/app/boekhouding/projecten/uren', icon: Clock, label: 'Urenregistratie' },
    ]
  }
};
          ...
          {!isSuperAdmin() && addonsLoaded && (
            <>
              ...
              {getOrderedModules().map((moduleSlug, index) => {
                ...
                // Special handling for Boekhouding module with dropdown submenus
                    ...
                    <div key={moduleSlug} className="mb-1">
                      {!isCollapsed && (
                        <div className={`module-group-header ${isFirst ? 'mt-2' : ''}`}>
                          ...
                          {config.name}
                          <span className="text-emerald-400 font-normal">(Gratis)</span>
                        </div>
                      )}
                      {isCollapsed && !isFirst && <div className="mt-3 mb-3 mx-2 border-t border-primary/10" />}
                      
                      {/* Main boekhouding items (no dropdown) */}
                      {boekhoudingMainItems.map((item) => (
                        <NavLink
                          key={item.to}
                          to={item.to}
                          onClick={() => setSidebarOpen(false)}
                      ...
                      <div>
                        <button
                          onClick={() => setInkoopOpen(!inkoopOpen)}
                          className={`nav-item w-full ${location.pathname.includes('/boekhouding/inkoop') ? 'active' : ''} ${isCollapsed ? 'justify-center' : 'justify-between'}`}
                          data-testid="nav-inkoop-dropdown"
                          title="Inkoop"
                        >
                          <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center' : ''}`}>
                            <ShoppingCart className="w-5 h-5 flex-shrink-0" />
                            {!isCollapsed && <span>Inkoop</span>}
                          </div>
                          {!isCollapsed && <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${inkoopOpen ? 'rotate-180' : ''}`} />}
                        </button>
                        {!isCollapsed && (
                          <div className={`overflow-hidden transition-all duration-200 ${inkoopOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="pl-4 mt-1 space-y-0.5">
                              {boekhoudingDropdowns.inkoop.items.map((item) => (
                                <NavLink
                                  key={item.to}
                                  to={item.to}
                                  onClick={() => setSidebarOpen(false)}
                      ...
                      <div>
                        <button
                          onClick={() => setVerkoopOpen(!verkoopOpen)}
                          className={`nav-item w-full ${location.pathname.includes('/boekhouding/verkoop') ? 'active' : ''} ${isCollapsed ? 'justify-center' : 'justify-between'}`}
                          data-testid="nav-verkoop-dropdown"
                          title="Verkoop"
                        >
                          <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center' : ''}`}>
                            <Banknote className="w-5 h-5 flex-shrink-0" />
                            {!isCollapsed && <span>Verkoop</span>}
                          </div>
                          {!isCollapsed && <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${verkoopOpen ? 'rotate-180' : ''}`} />}
                        </button>
                        {!isCollapsed && (
                          <div className={`overflow-hidden transition-all duration-200 ${verkoopOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="pl-4 mt-1 space-y-0.5">
                              {boekhoudingDropdowns.verkoop.items.map((item) => (
                                <NavLink
                                  key={item.to}
                                  to={item.to}
                                  onClick={() => setSidebarOpen(false)}
                      ...
                      <div>
                        <button
                          onClick={() => setVoorraadOpen(!voorraadOpen)}
                          className={`nav-item w-full ${location.pathname.includes('/boekhouding/voorraad') ? 'active' : ''} ${isCollapsed ? 'justify-center' : 'justify-between'}`}
                          data-testid="nav-voorraad-dropdown"
                          title="Voorraad"
                        >
                          <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center' : ''}`}>
                            <Package className="w-5 h-5 flex-shrink-0" />
                            {!isCollapsed && <span>Voorraad</span>}
                          </div>
                          {!isCollapsed && <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${voorraadOpen ? 'rotate-180' : ''}`} />}
                        </button>
                        {!isCollapsed && (
                          <div className={`overflow-hidden transition-all duration-200 ${voorraadOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="pl-4 mt-1 space-y-0.5">
                              {boekhoudingDropdowns.voorraad.items.map((item) => (
                                <NavLink
                                  key={item.to}
                                  to={item.to}
                                  onClick={() => setSidebarOpen(false)}
                      ...
                      <div>
                        <button
                          onClick={() => setProjectenOpen(!projectenOpen)}
                          className={`nav-item w-full ${location.pathname.includes('/boekhouding/projecten') ? 'active' : ''} ${isCollapsed ? 'justify-center' : 'justify-between'}`}
                          data-testid="nav-projecten-dropdown"
                          title="Projecten"
                        >
                          <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center' : ''}`}>
                            <Briefcase className="w-5 h-5 flex-shrink-0" />
                            {!isCollapsed && <span>Projecten</span>}
                          </div>
                          {!isCollapsed && <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${projectenOpen ? 'rotate-180' : ''}`} />}
                        </button>
                        {!isCollapsed && (
                          <div className={`overflow-hidden transition-all duration-200 ${projectenOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="pl-4 mt-1 space-y-0.5">
                              {boekhoudingDropdowns.projecten.items.map((item) => (
                                <NavLink
                                  key={item.to}
                                  to={item.to}
                                  onClick={() => setSidebarOpen(false)}
                      ...
                      {/* Footer items (Rapportages, Handleiding) */}
                      {boekhoudingFooterItems.map((item) => (
                        <NavLink
                          key={item.to}
                          to={item.to}
                          onClick={() => setSidebarOpen(false)}
